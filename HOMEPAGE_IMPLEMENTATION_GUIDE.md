# 🏠 Guia de Implementação - Nova Homepage Atma

## 📋 Status da Implementação

### ✅ Componentes Criados
- `Frontend/components/ui/ticker-tape.tsx` - Banner rotativo de mensagens
- `Frontend/components/ui/step-carousel.tsx` - Carrossel de 3 etapas do processo
- `Frontend/components/ui/doctor-locator.tsx` - Formulário de busca de ortodontistas

### 📄 Documentação
- `HOMEPAGE_REDESIGN_STRATEGY.md` - Estratégia completa baseada em análise Invisalign

### ⏳ Próximas Etapas
1. Substituir `Frontend/app/page.tsx` com novo código
2. Coletar fotos reais de pacientes (com autorização)
3. Criar rotas faltantes (/quiz, /contencoes, páginas por idade)
4. Configurar tracking de eventos GA4

---

## 🎯 Código Completo para `Frontend/app/page.tsx`
...
import { Check, Star, Clock, DollarSign, Award, Users, Sparkles, ChevronRight, MapPin } from "lucide-react"
...
### Fotos para Substituir Placeholders:

1. **Hero Section:**
   - `/placeholder-patient-smiling.jpg` → Paciente real feliz usando alinhador
   - Resolução: 600x700px mínimo
   - Formato: WebP ou AVIF (otimizado)

2. **Step Carousel:**
   - `/placeholder-scan.jpg` → Foto do escaneamento 3D intraoral
   - `/placeholder-aligners.jpg` → Foto dos alinhadores Atma no estojo
   - `/placeholder-result.jpg` → Antes/depois real de paciente

3. **Social Proof:**
   - 3 fotos de antes/depois autorizadas
   - Resolução: 400x400px (quadradas)

4. **Tecnologia:**
   - `/placeholder-technology.jpg` → Material PETG + certificados ISO/CE/ANVISA
   - Resolução: 600x400px

### Requisitos Legais:
- ✅ Termo de autorização de uso de imagem assinado
- ✅ Proteção de dados pessoais (LGPD)
- ✅ Não usar fotos de bancos de imagens genéricos

---

## 🎯 Próximos Passos

### Fase 1: Implementação Base (2-3 horas)
1. Substituir `Frontend/app/page.tsx` com código acima
2. Testar responsividade mobile
3. Verificar todos os links internos

### Fase 2: Assets Reais (1 semana)
1. Coletar fotos de pacientes (com autorização)
2. Fotografar produtos (alinhadores, estojo, scanner)
3. Otimizar imagens para Web (WebP/AVIF)

### Fase 3: Rotas Faltantes (4-5 horas)
1. Criar `/pacientes/criancas`
2. Criar `/pacientes/adolescentes`
3. Criar `/pacientes/adultos`
4. Criar `/quiz` (formulário de avaliação)
5. Criar `/tecnologia` (detalhes técnicos)
6. Criar `/ortodontistas/parceria` (aplicação B2B)

### Fase 4: Analytics & Otimização (2 horas)
1. Configurar eventos GA4 para todos os CTAs
2. Configurar pixel Meta/Google Ads
3. Lighthouse performance audit
4. Correções de acessibilidade (WCAG)

### Fase 5: Teste A/B (2 semanas)
...
## 📊 KPIs Esperados (60 dias pós-lançamento)

| Métrica | Antes | Meta | Melhoria |
|---------|-------|------|----------|
| Taxa de conversão (pacientes) | 2,3% | 5-7% | +220% |
| Taxa de conversão (ortodontistas) | 0% | 1-2% | ∞ |
| Tempo na página | 45s | 2m30s | +233% |
| Taxa de rejeição | 68% | 45% | -34% |
| Leads qualificados/mês | 32 | 150+ | +369% |

---

## ✅ Checklist de Lançamento

- [ ] Código implementado em `page.tsx`
- [ ] Componentes testados (TickerTape, StepCarousel, DoctorLocator)
- [ ] Fotos reais coletadas e otimizadas
- [ ] Rotas criadas (/quiz, /criancas, /adolescentes, /adultos, /tecnologia, /parceria)
- [ ] GA4 configurado com eventos personalizados
- [ ] Meta tags e Schema.org atualizados
- [ ] Lighthouse score > 90 (Performance, SEO, Accessibility)
- [ ] Testes em mobile (iOS Safari, Android Chrome)
- [ ] Testes em desktop (Chrome, Firefox, Safari, Edge)
- [ ] Deploy em staging para review
...
**Status:** ✅ Componentes prontos | ⏳ Aguardando implementação em page.tsx
